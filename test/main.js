...
describe('gulp-wrap-layout', function () {
  ...
  it('should error when no template is provided', function () {
    (function(){
      wrap();
    }).should.throw('gulp-wrap-layout: Missing template parameter');
  });
  ...
  it('should handle a template from a file src via buffer', function (done) {
    ...
    var stream = wrap({src: 'template.txt'});
  ...
  it('should find a template in the parent diretory of the source file', function (done) {

    var srcFile = new gutil.File({
      path: 'test/fixtures/deep/hello.txt',
      cwd: 'test/',
      base: 'test/fixtures/deep',
      contents: fs.readFileSync('test/fixtures/deep/hello.txt')
    });
  ...
  it('should throw an exception if the template file does not exist anywhere above the source file directory', function (done) {
    ...
    var stream = wrap({src: 'templateWhichDoesNotExist.txt'});
  ...
  it('should handle template data and options', function (done) {
    ...
    var stream = wrap('BEFORE <%= contents %> <%= data.someVar %> AFTER', { someVar: 'someVal'});
  ...
  it('should not pollute file data across multiple streams', function (done) {
    ...
    stream.on('data', function (newFile) {
      ...
      if (newFile.relative === 'hello.txt') {
        String(newFile.contents).should.equal('Contents: [Hello] - File prop from 1: [someValueFrom1] - File prop from 2: [undefined]');
      } else if (newFile.relative === 'hello2.txt') {
        String(newFile.contents).should.equal('Contents: [Hello2] - File prop from 1: [undefined] - File prop from 2: [someValueFrom2]');
      }
    });
